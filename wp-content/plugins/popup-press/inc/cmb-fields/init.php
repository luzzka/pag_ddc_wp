...
spl_autoload_register('cmb_Meta_Box_PPS::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'cmb_meta_boxes_pps', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new cmb_Meta_Box_PPS( $meta_box );
}

define( 'CMB_META_BOX_URL_PPS', cmb_Meta_Box_PPS::get_meta_box_url() );

/**
 * Create meta boxes
 */
class cmb_Meta_Box_PPS {
	...
	function __construct( $meta_box ) {
		...
		$show_filters = 'cmb_Meta_Box_PPS_Show_Filters';
	...
	public function register_scripts() {
		...
		wp_register_script( 'cmb-datepicker', CMB_META_BOX_URL_PPS . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'cmb-timepicker', CMB_META_BOX_URL_PPS . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'cmb-scripts-pps', CMB_META_BOX_URL_PPS .'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );

		wp_enqueue_media();

		wp_localize_script( 'cmb-scripts-pps', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
		...
		wp_register_style( 'cmb-styles-pps', CMB_META_BOX_URL_PPS . 'style'. $min .'.css', $styles );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new cmb_Meta_Box_PPS_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new cmb_Meta_Box_PPS_field( $args );
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new cmb_Meta_Box_PPS_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
		  ...
		  //  $cmb_url = str_replace(
			...
			// 	dirname( __FILE__ )
			// );
			$cmb_url = plugin_dir_url( __FILE__ );
		}

		$cmb_url = set_url_scheme( $cmb_url );

		return trailingslashit( apply_filters('cmb_meta_box_url_pps', $cmb_url ) );
	}
...
// Handle oembed Ajax
add_action( 'wp_ajax_cmb_oembed_handler', array( 'cmb_Meta_Box_PPS_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cmb_oembed_handler', array( 'cmb_Meta_Box_PPS_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function cmb_get_option_PPS( $option_key, $field_id = '' ) {
	return cmb_Meta_Box_PPS::get_option( $option_key, $field_id );
}
 ...
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              cmb_Meta_Box_PPS_field object
 */
function cmb_get_field_PPS( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	cmb_Meta_Box_PPS::set_object_id( $object_id );
	cmb_Meta_Box_PPS::set_object_type( $object_type );
	// Send back field object
	return new cmb_Meta_Box_PPS_field( $field_args );
}
 ...
 */
function cmb_get_field_value_PPS( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = cmb_get_field_PPS( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function cmb_print_metaboxes_PPS( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		cmb_print_metabox_PPS( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function cmb_print_metabox_PPS( $meta_box, $object_id ) {
	$cmb = new cmb_Meta_Box_PPS( $meta_box );
	if ( $cmb ) {

		cmb_Meta_Box_PPS::set_object_id( $object_id );

		if ( ! wp_script_is( 'cmb-scripts-pps', 'registered' ) )
		...
		cmb_Meta_Box_PPS::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function cmb_save_metabox_fields_PPS( $meta_box, $object_id ) {
	cmb_Meta_Box_PPS::save_fields( $meta_box, $object_id );
}
 ...
 */
function cmb_metabox_form_PPS( $meta_box, $object_id, $echo = true ) {

	$meta_box = cmb_Meta_Box_PPS::set_mb_defaults( $meta_box );
	...
	cmb_Meta_Box_PPS::set_object_type( cmb_Meta_Box_PPS::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], cmb_Meta_Box_PPS::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		cmb_save_metabox_fields_PPS( $meta_box, $object_id );
