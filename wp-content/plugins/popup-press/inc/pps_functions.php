...
function get_popup_PPS($popup_id = 0){
	...
	$popup = '';
	$popup .= "<div id='popuppress-$popup_id' class='pps-popup $has_border $size_popup' data-popuppress='$data_json'>";
...
function get_script_popup_PPS($popup_id = 0){
	...
	$close_overlay = ($disclaimer_activate == 'true' && !is_admin()) ? 'false' : get_post_meta_PPS( $popup_id, 'pps_close_overlay' );
	...
	$bPopup = '
			pps_popup_'.$popup_id.' = jQuery("#popuppress-'.$popup_id.'").bPopup({
				...
				onOpen: function(){
					...
					onOpenPopupPress('.$popup_id.', "'.$img_overlay.'");
...
function get_image_content_PPS(&$numItems, $popup_id, $pps_options){
	...
	if(!empty($pps_rg_img)){
		foreach ($pps_rg_img as $key => $img_array) {
            ...
            $content .= '<li class="pps-item-content">'.$link_star.'<img src="'.$img_file.'" class="pps-img-slider" />'.$link_end . $img_caption .'</li>';
					$numItems++;
			}
		}
	}
	...
	if( ! empty( $pps_rg_oembed ) ) {
		foreach ($pps_rg_oembed as $key => $oemded_array) {
			...
			if( $embed_code ){
				...
				$content .= "<li class='pps-item-content'><div class='pps-embed' data-src='$embed_src'>$embed_code</div></li>";
				$numItems++;
			}
		}
	}
...
function get_iframe_content_PPS($pps_data, &$numItems, $popup_id){
	...
	if( !empty($iframe_content)) {
		$content = '<li class="pps-item-content"><div class="pps-iframe" data-src="'.$iframe_content.'" data-height="'.$iframe_height.'"><iframe src="about:blank" height="'.$iframe_height.'"></iframe></div></li>';
		$numItems++;
	}
	return $content;
}
